...
/* returns size in sectors or 0 on error? */
int reg_info(unsigned char drvNum, char *buf)
{
    outb(0x1f6, (drvNum ? CB_DH_DEV1 : CB_DH_DEV0)); /* select drive */
    outb(0x1f7, 0xEC); /* get drive data */

    tmr_set_timeout();

    while (inb(0x1f7) != 0x58) if (tmr_chk_timeout()) return 0;
...
{
    L4_Word64_t raw = L4_SystemClock().raw;

    raw /= (1000000UL / SYSTEM_TIMER_TICKS_PER_SECOND);
...

{
   ...
   return inb((unsigned long) pio_reg_addrs[addr]);
}
...

{
   ...
   outb((unsigned long) pio_reg_addrs[addr], data);
}
...

{
   ...
   return inw((unsigned long) pio_reg_addrs[addr]);
}
...

{
  ...
  // * ( (unsigned int *) pio_reg_addrs[ addr ] ) = data;
   outw((unsigned long) pio_reg_addrs[addr], data);
}
...

{
   ...
   return inl((unsigned long) pio_reg_addrs[addr]);
}
...

{
   ...
   * ( (unsigned long *) pio_reg_addrs[ addr ] ) = data;
   outl((unsigned long) pio_reg_addrs[addr], data);
}
