...
#include <stdio.h>
/* This is Hasenpfeffer-specific */
static unsigned char *heap_start = (unsigned char *) 0x80000000UL; /* 2GB - start of heap area */
Header *morecore(unsigned units)
{
    /* touch it now */
    volatile unsigned char x = *heap_start; x++;

    Header *tmp = (Header *) heap_start;
    heap_start += units * sizeof(Header);

    tmp->s.ptr = tmp;
    tmp->s.size = units;
    printf("returning %p, request was %d bytes\n", tmp, units * sizeof(Header));
    return tmp;
}
...
/* all of this is sooo full of crap...:( */
#include <stdio.h>
/* This is Hasenpfeffer-specific */
static unsigned char *heap_start = (unsigned char *) 0x80000000UL; /* 2GB - start of heap area */
void *malloc(unsigned long size)
{
    ...
    // make it aligned
    while ((L4_Word_t) heap_start % 4) heap_start++;
